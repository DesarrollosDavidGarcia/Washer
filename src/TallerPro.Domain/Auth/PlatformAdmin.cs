...
/// <summary>Platform-level administrator — separate entity from User, not tenant-scoped.</summary>
public sealed class PlatformAdmin : IAuditable, ISoftDeletable, IConcurrencyAware
{
    private static readonly Regex EmailRegex =
        new(@"^[^@\s]+@[^@\s]+\.[^@\s]+$", RegexOptions.Compiled, TimeSpan.FromSeconds(1));

    public PlatformAdmin(string email, string displayName)
    {
